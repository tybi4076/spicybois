...
milkMost <- milk %>%
  filter(year == 2017) %>%
  group_by(state) %>%
  summarise(count = sum(milk_produced), countMil = sum(milk_million)) %>%
  arrange(desc(countMil))

milkMost

```

**Findings:** The 5 states that produced the Most milk in 2017 were California, Wisconsin, New York, Idaho, and Texas
...
**Findings:** The 5 States that produced the least amount of milk in 2017 were Alaska, Rhode Island, Hawaii, Arkansas, and Alabama.
...
**Jack:** I created the plot for the milk productin in Gerogia. For the Georgia plot I used the filter function to only utlize data from the state of georgia.
